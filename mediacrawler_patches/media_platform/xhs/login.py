...
class XiaoHongShuLogin(AbstractLogin):
    ...
    async def _browser_shows_logged_in_profile(self) -> bool:
        """
        True if the top nav already shows the profile entry (已登入時不會出現掃碼框).
        Used when API pong() is False but Chrome/CDP session is already logged in.
        """
        for xp in (
            "xpath=//a[contains(@href, '/user/profile/')]//span[text()='我']",
            "xpath=//a[contains(@href, '/user/profile/')]//span[text()='Me']",
        ):
            try:
                if await self.context_page.is_visible(xp, timeout=2000):
                    return True
            except Exception:
                continue
        return False
    ...
    async def login_by_qrcode(self):
        ...
        if await self._browser_shows_logged_in_profile():
            utils.logger.info(
                "[XiaoHongShuLogin.login_by_qrcode] Page already shows logged-in profile (no QR layer); "
                "skipping QR — often API pong() is false while browser session is valid."
            )
            return
        ...
        if not base64_qrcode_img:
            ...
            if await self._browser_shows_logged_in_profile():
                utils.logger.info(
                    "[XiaoHongShuLogin.login_by_qrcode] No qrcode but profile visible after first wait — already logged in."
                )
                return
            ...
            if not base64_qrcode_img:
                if await self._browser_shows_logged_in_profile():
                    utils.logger.info(
                        "[XiaoHongShuLogin.login_by_qrcode] No qrcode img but profile visible — treating as logged in."
                    )
                    return
                sys.exit()
